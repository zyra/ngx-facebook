...
export class FBShareComponent extends FBMLComponent {
   ...
   * Can be one of `box_count`, `button_count`, `button`. Defaults to `icon_link`.
   */
  @Input()
  @FBMLAttribute
  layout: string;
  ...
  /**
   * If set to true, the share button will open the share dialog in an iframe (instead of a popup)
   * on top of your website on mobile. This option is only available for mobile, not desktop. Defaults to `false`.
   */
  @Input()
  @FBMLAttribute
