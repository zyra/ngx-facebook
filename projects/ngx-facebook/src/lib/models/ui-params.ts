...
export interface ShareDialogParams {
   ...
   * The hashtag should include the hash symbol, e.g. #facebook.
   */
  hashtag?: string;

  /**
   * A quote to be shared along with the link, either highlighted by the user or predefined by the developer,
   * as in a pull quote on an article.
   */
  quote?: string;

  /**
   * If set to true the share button will open the share dialog in an iframe on top of your website
   * (For more information see Mobile Web Share Dialog. This option is only available for mobile, not desktop.
   */
  mobile_iframe?: string;

  /**
   * A string specifying which Open Graph action type to publish, e.g., og.likes for the built in like type.
   * The dialog also supports approved custom types. Required when using method share_open_graph.
   */
  action_type?: string;

  /**
   * A JSON object of key/value pairs specifying parameters which correspond to the action_type being used.
   * Valid key/value pairs are the same parameters that can be used when publishing Open Graph Actions using the API.
...
export interface FeedDialogParams {
  ...
  /**
   * The ID of the profile that this story will be published to. If this is unspecified,
   * it defaults to the value of from. The ID must be a friend who also uses your app.
   */
  to?: string;

  /**
   * The link attached to this post. With the Feed Dialog, people can also share plain text status
   * updates with no content from your app; just leave the link parameter empty.
   */
  link?: string;
   ...
   * each of length at least 1 and at most 15 characters drawn from the
...
/**
 * The object passed to the [ui](../facebook-service/#ui) method.
 * The properties below can be passed to all three types of dialogs.
 * Each type of dialog accepts additional parameters that are documented above.
 */
export interface UIParams extends ShareDialogParams, FeedDialogParams, SendDialogParams {
   ...
   * be a full page display, shown within Facebook.com. This display type is called page
   ...
   * - If you are using the Facebook SDK for JavaScript, this will default to a modal iframe type
   * for people logged into your app or async when using within a game on Facebook.com,
   * and a popup window for everyone else. You can also force the popup type when when you use the
   * Facebook SDK for JavaScript, if necessary.
   * - Mobile web apps will always default to the touch display type.
   */
  display?: string;

  /**
   * - Dialog in create phase allows you to get stream url to upload video;
   * Dialog in publish phase will provide preview and go live button. required
